...
export function clearTmdbCache() {
  _tmdbCache.clear();
  try {
    localStorage.removeItem("watchalong_trendingCache");
  } catch {}
}
...
export const tmdbFetch = async (path, apiKey) => {
  ...
  let res;
  try {
    res = await fetch(`/api/tmdb${localizedPath}`, {
...
const ANILIST_CACHE_KEY = "watchalong_anilistCache";
...
const EG_CACHE_KEY = "watchalong_episodeGroupCache";
