...
function VersionSection() {
  ...
  // Updates are a desktop-app feature; not applicable to the web build.
...
function BackupRestoreSection({ onRestored }) {
  ...
  const handleExport = () => {
    ...
    a.href = url;
    a.download = `watchalong-backup-${new Date().toISOString().slice(0, 10)}.json`;
    a.click();
...
function AppearanceSection() {
  ...
  const handleSave = () => {
    ...
    window.dispatchEvent(new CustomEvent("watchalong:gamepad-settings-changed"));
    ...
    window.dispatchEvent(new CustomEvent("watchalong:player-settings-changed"));
    setSaved(true);
    setTimeout(() => setSaved(false), 2000);
  };
      ...
      <div style={{ marginBottom: 24 }}>
        ...
        >
          ...
          <div>
            ...
            <div style={{ fontSize: 12, color: "var(--text3)", marginTop: 2 }}>
              Applies the selected accent colour to the streaming player when
              supported.
            </div>
          </div>
        </div>
...
function LibraryPrivacySection() {
  ...
  const handleSave = () => {
    ...
    window.dispatchEvent(
      new CustomEvent("watchalong:library-sort-changed", { detail: sort }),
    );
    setSaved(true);
    setTimeout(() => setSaved(false), 2000);
  };
...
function MetadataLanguageSection() {
  ...
  const handleSave = () => {
    ...
    window.dispatchEvent(new CustomEvent("watchalong:tmdb-lang-changed"));
    setSaved(true);
    setTimeout(() => setSaved(false), 2000);
  };
  ...
  return (
    <div style={{ marginBottom: 40 }}>
      ...
      >
        ...
        Changing this clears the metadata cache so updated content loads
        immediately.
      </div>
      <div
        style={{
...
function DiscordRpcSection() {
  ...
  const saveSettings = () => {
    ...
    window.dispatchEvent(
      new CustomEvent("watchalong:discord-rpc-settings-changed"),
    );
    setSaved(true);
    setTimeout(() => setSaved(false), 2000);
  };
    ...
    <div style={{ marginBottom: 40 }}>
      ...
      >
        ...
        <ToggleRow
          ...
          description="Uses the movie/show poster as the large image. When off, only the WatchAlong icon is shown."
          value={showCover}
        ...
        <ToggleRow
          ...
          description="Adds a button to your Discord status card."
          value={showButton}
      ...
      >
        ...
        <div ref={navRef} style={{ position: "relative", flexShrink: 0 }}>
          ...
          {navOpen && (
            ...
            >
              {SECTION_NAV.filter(
                (s) => isElectron || !["downloads", "discordRpc", "backup"].includes(s.id)
              ).map((s) => (
                <button
                  key={s.id}
...
export default function SettingsPage({
  ...
  const [saved, setSaved] = useState(false);
  const [redirectShield, setRedirectShield] = useState(
    () => storage.get(STORAGE_KEYS.REDIRECT_SHIELD) !== false,
  );
    ...
    <>
      ...
      {showDeleteDlConfirm && (
        <ConfirmDialog
          ...
          description="This will permanently delete all video files downloaded through WatchAlong and remove them from the download list."
      ...
      >
        ...
        <div style={{ color: "var(--text3)", fontSize: 14, marginBottom: 48 }}>
          App configuration for WatchAlong
        </div>
        ...
        <div ref={secUpdates} style={{ scrollMarginTop: 80 }}>
          <SectionGroupHeader
            title="General"
            subtitle="App version, updates and Languages"
          />
        ...
        <div ref={secPlayback} style={{ scrollMarginTop: 80 }}>
          <SectionGroupHeader
            title="Playback"
            subtitle="Playing, auto-watched and skip behavior"
          />
          ...
          <div style={{ marginBottom: 40 }}>
            <ToggleRow
              ...
              description="Watches the player for ad-redirect loops and blanks it with a 'Redirect blocked' prompt before a sketchy page chain can continue. Popups are handled by the browser's built-in popup blocker. Turn off if you see false positives."
              value={redirectShield}
              onChange={(v) => {
                setRedirectShield(v);
                storage.set(STORAGE_KEYS.REDIRECT_SHIELD, v);
              }}
            />
          </div>
        </div>
        ...
        <div ref={secSubtitles} style={{ scrollMarginTop: 80 }}>
          <SectionGroupHeader
            title="Subtitles"
            subtitle="Preferred subtitle language for video downloads"
          />
          <SubtitleSettingsSection />
        </div>

        {/* ══════════════════════════════════════════════════════════════════ */}
        {/* GROUP: DOWNLOADS (desktop only)                                    */}
        {/* ══════════════════════════════════════════════════════════════════ */}
        {isElectron && (
          ...
          <div style={{ marginBottom: 40 }}>
            ...
            >
              ...
              and WatchAlong has write access to it.
            </div>
            <div
              style={{
                display: "flex",
        ...
        {/* ══════════════════════════════════════════════════════════════════ */}
        {/* GROUP: DISCORD RICH PRESENCE (desktop only)                        */}
        {/* ══════════════════════════════════════════════════════════════════ */}
        {isElectron && (
          ...
          >
            ...
            <div style={{ padding: "22px 24px" }}>
              <CleanRow
                ...
                description="Opens the folder where WatchAlong is installed."
            ...
            <div style={{ padding: "22px 24px" }}>
              <CleanRow
                title="Delete All Downloads"
                description="Permanently deletes all video files that were downloaded through WatchAlong and removes them from the download list. Only files downloaded through the app will be deleted, nothing else in your folder is touched."
            ...
            >
              ...
              >
                <div style={{ flex: 1 }}>
                  ...
                  >
                    Completely resets WatchAlong to factory defaults, clears all
                    settings, saved library, watch history/progress, and all
                    cached data. Your downloaded video files will not be
                    touched.
                  </div>
                </div>
                <div style={{ flexShrink: 0, paddingTop: 2 }}>
